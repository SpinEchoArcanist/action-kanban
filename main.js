...
Action Kanban — Version 11.5
*/

"use strict";
var AK_BUILD_VERSION = "11.5";
...
var ObsidianActionRepository = class _Repo {
  ...
  async createAutoNamedNote() {
    const folder = this.getActionsFolder();
    await this._ensureFolder(folder);
    const path = this.app.vault.getAvailablePath(`${folder}/Untitled`, "md");
    await this.app.vault.create(path, "");
    return path;
  }
...
function makeSingleChevronSvg(color, rotate) {
  ...
  if (rotate) svg.style.transform = "rotate(180deg)";
  const setAttrs = el => {
  ...
  top.setAttribute("points", "2,9 7,4 12,9");
  setAttrs(top);
  const bot = document.createElementNS(ns, "polyline");
  bot.setAttribute("points", "2,13 7,8 12,13");
  setAttrs(bot);
  svg.appendChild(top);
  svg.appendChild(bot);
  return svg;
...
var KanbanBoardRenderer = class {
  ...
  renderToolbar(container, refresh, settings) {
    ...
    const poly = document.createElementNS(ns, "polyline"); poly.setAttribute("points", "23 4 23 10 17 10");
    const pth  = document.createElementNS(ns, "path");     pth.setAttribute("d", "M20.49 15a9 9 0 1 1-2.12-9.36L23 10");
    svg.appendChild(poly); svg.appendChild(pth);
    ...
    FolderSuggest = class extends import_obsidian6.AbstractInputSuggest {
      ...
      getSuggestions(query) {
        ...
        for (const f of this.app.vault.getAllLoadedFiles()) {
          if (f instanceof import_obsidian6.TFolder && f.path.toLowerCase().includes(q)) {
            folders.push(f.path);
          }
        }
...
var ActionKanbanSettingTab = class extends import_obsidian6.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.createEl("h2", { text: "Action Kanban Settings" });
    ...
    containerEl.createEl("h3", { text: "Note identification" });
    new import_obsidian6.Setting(containerEl)
    ...
    containerEl.createEl("h3", { text: "Board behaviour" });
    new import_obsidian6.Setting(containerEl)
    ...
    containerEl.createEl("h3", { text: "Card meta fields" });
    containerEl.createEl("p", {
      cls: "ak-settings-hint",
  ...
  _renderColumnRows(container, validationEl) {
    ...
    this.plugin.settings.columns.forEach((col, idx) => {
      ...
      for (const status of this.plugin.settings.statuses) {
        ...
        lbl.htmlFor = cb.id;
        if (isPlaceholder) lbl.style.color = "var(--text-faint)";
      }
...
var ActionKanbanPlugin = class extends import_obsidian5.Plugin {
  async onload() {
    console.log(`[Action Kanban] build ${AK_BUILD_VERSION} (manifest v${this.manifest.version}) loaded`);
    ...
    this.addRibbonIcon("layout-dashboard", "Open Action Kanban", () => this._openKanbanView());
    this.addCommand({ id: "open-kanban",    name: "Open Action Kanban",   callback: () => this._openKanbanView() });
    this.addCommand({ id: "new-action",     name: "New Action Note",      callback: async () => { await createAction(); this._refreshAllViews(); } });
    this.addCommand({ id: "refresh-kanban", name: "Refresh Kanban Board", callback: () => this._refreshAllViews() });
    this.addCommand({
      id: "embed-kanban",
      name: "Embed Action Kanban board",
    ...
    this.addCommand({
      ...
      name: "Embed Action Kanban board in a callout",
      editorCallback: (editor) => {
    ...
    this.registerMarkdownCodeBlockProcessor("action-kanban", async (source, el, ctx) => {
      ...
      child.register(() => { if (debounceTimer) clearTimeout(debounceTimer); });
      ctx.addChild(child);
    });
